...
namespace ts {
    ...
    public:
        ...
        simd(base a, base b, base c, base d);

        void store(base *p) const;
    };

    simd<float, 4>::simd(const base *p) : value(_simd_f32x4_load(p)) {}

    simd<float, 4>::simd(base a, base b, base c, base d) : value(_simd_f32x4_set(a, b, c, d)) {}

    void simd<float, 4>::store(base *p) const { _simd_f32x4_store(p, value); }
