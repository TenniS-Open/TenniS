...
namespace ts {
    namespace base {
        ...
        void Conv2D::init() {
            ...
            auto dialations_tensor = tensor::cast(INT32, get(name::dialations));

            TS_AUTO_CHECK(padding_tensor.has_shape({4, 2}));
            TS_AUTO_CHECK(stride_tensor.has_shape({4,}));
            TS_AUTO_CHECK(dialations_tensor.has_shape({4,}));
            ...
            m_dialations4.resize(4);
            for (size_t i = 0; i < 4; ++i) m_dialations4[i] = dialations_tensor.data<int32_t>(i);
            ...
            if (m_format == FORMAT_NCHW) {
                ...
                if (m_dialations4[0] != 1 ||
                    m_dialations4[1] != 1) {
                    TS_LOG_ERROR << this->op() << " do not support dialations: " << to_string(m_dialations4) << eject;
                }
        ...
        int Conv2D::infer(Stack &stack, std::vector<Tensor::Prototype> &output) {
            ...
            if (m_format == FORMAT_NCHW) {
                ...
                dialations = Stride2D(m_dialations4[2], m_dialations4[3]);
            } else if (m_format == FORMAT_NHWC) {
