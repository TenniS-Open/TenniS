...
namespace ts {
    namespace base {
        ...
        int SliceV3::run(Stack &stack) {
            ...
            slice(raw_in, begin, end, stride, raw_out);
        ...
        void SliceV3::load_params(const Stack &stack) {
            ...
            if (stack.size() > 3) {
                ...
                axes = std::vector<int32_t>(starts.size());
            ...
            if (stack.size() > 4) {
                ...
                steps = std::vector<int32_t>(slice_size, 1);
            }
            ...
            m_begin = std::vector<int32_t>(max_dims, 0);
            m_end = std::vector<int32_t>(max_dims, 0);
            m_stride = std::vector<int32_t>(max_dims, 1);
