...
namespace ts {
    namespace gpu {
        ...
        template<typename T>
        static inline void sub_gpu_compute_run(const Tensor &lhs, const Tensor &rhs, Tensor &out) {
        ...
        template<typename T>
        static inline void sub_gpu_compute_run_scalar(const Tensor &lhs, const Tensor &rhs, Tensor &out) {
            auto plhs = lhs.data<T>();
            auto prhs = rhs.data<T>();
            auto pout = out.data<T>();
        ...
        template<typename T>
        static inline void sub_gpu_compute_run_bias(const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
            auto plhs = lhs.data<T>();
            auto prhs = rhs.data<T>();
            auto pout = out.data<T>();
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { sub_gpu_compute_run<TYPE>(lhs, rhs, out); break; }
...
#undef DECLARE_COMPUTE_RUN
        ...
        void Sub::reduce_with_scalar(const Tensor &lhs, const Tensor &rhs, Tensor &out) {
            ...
            switch(dtype) {
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { sub_gpu_compute_run_scalar<TYPE>(lhs, rhs, out); break; }
                DECLARE_COMPUTE_RUN(INT8, int8_t);
                DECLARE_COMPUTE_RUN(UINT8, uint8_t);
                DECLARE_COMPUTE_RUN(INT16, int16_t);
                DECLARE_COMPUTE_RUN(UINT16, uint16_t);
...
#undef DECLARE_COMPUTE_RUN
        ...
        void Sub::reduce_with_bias(const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
            ...
            switch(dtype) {
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { sub_gpu_compute_run_bias<TYPE>(lhs, rhs, out, dim); break; }
                DECLARE_COMPUTE_RUN(INT8, int8_t);
                DECLARE_COMPUTE_RUN(UINT8, uint8_t);
                DECLARE_COMPUTE_RUN(INT16, int16_t);
                DECLARE_COMPUTE_RUN(UINT16, uint16_t);
...
using namespace ts;
using namespace gpu;
TS_REGISTER_OPERATOR(Sub, GPU, name::layer::sub())
#ifdef TS_USE_CUDA_FP16
TS_REGISTER_FP16_OPERATOR(Sub, GPU, name::layer::sub())
#endif
