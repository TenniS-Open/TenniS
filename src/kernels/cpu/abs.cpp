...
namespace ts {
    namespace cpu {
        template<typename T>
        static void cpu_abs_compute_run(const Tensor &x, Tensor &out) {
        ...
        class Abs : public OperatorOnCPU<base::Activation> {
        public:
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { cpu_abs_compute_run<TYPE>(x, out); break; }
...
using namespace ts;
using namespace cpu;
TS_REGISTER_OPERATOR(Abs, CPU, "abs")