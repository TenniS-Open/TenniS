...
namespace ts {
    namespace cpu {
        template<typename T>
        static void cpu_inner_prod_compute_run(const Tensor &lhs, const Tensor &rhs, Tensor &out) {
            ...
            T *pdst = out.data<T>();
#ifdef TS_USE_CBLAS
            cblas::math<T>::gemm(blas::NoTrans, blas::NoTrans, lhs_shape[0], rhs_shape[1], lhs_shape[1],
                                 (T) 1, psrc, pdot, (T) 0, pdst);
#else
            cpu::math<T>::gemm(blas::NoTrans, blas::NoTrans, lhs_shape[0], rhs_shape[1], lhs_shape[1],
                               (T) 1, psrc, pdot, (T) 0, pdst);
#endif
        }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { cpu_inner_prod_compute_run<TYPE>(lhs, rhs, out); break; }
