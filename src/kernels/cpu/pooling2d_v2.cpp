...
namespace ts {
	...
	int Pooling2dV2::infer(ts::Stack &stack, std::vector<ts::Tensor::Prototype> &output)
	{
		...
		Size2D output_size;
		output_size = pooling2d_forward(input_size,m_padding, ksize, stride);
	...
	template<typename T>
	bool Pooling2dV2::pooling(ts::Stack &stack)
	{
		...
		{
			flag = max_pooling<T>(input_data, output_data, input_shape, output_shape, ksize, stride);
		}
		else
		{
			flag = average_pooling<T>(input_data, output_data, input_shape, output_shape, ksize, stride);
		}
		return true;
	}

	template<typename T>
	bool Pooling2dV2::max_pooling(T* input_data, T* output_data, Shape& input_shape, Shape& output_shape, KSize2D& ksize, Stride2D& stride)
	{
	...
	template<typename T>
	bool Pooling2dV2::average_pooling(T* input_data, T* output_data, Shape& input_shape, Shape& output_shape, KSize2D& ksize, Stride2D& stride)
	{
		int input_h = input_shape[2];
		int input_w = input_shape[3];
		int output_h = output_shape[2];
