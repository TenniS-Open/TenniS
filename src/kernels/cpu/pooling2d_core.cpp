    ...
    namespace cpu {
        ...
        void Pooling2DCore::pooling2d(const Tensor &x, Pooling2DType type, const Padding2D &padding,
            ...
            if (padding_type != Padding2DType::BLACK) {
                TS_LOG_ERROR << "Pooling2D only support black padding" << eject;
            }
...
#undef DECLARE_COMPUTE_RUN
                default: {
                    TS_LOG_ERROR << "Pooling2D not support this data type: " << type_str(dtype) << eject;
                    break;
                }
            }
        }
