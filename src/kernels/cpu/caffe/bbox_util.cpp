...
#define CHECK_EQ TS_AUTO_CHECK_EQ
#define CHECK_NE TS_AUTO_CHECK_NQ
#define CHECK_GT TS_AUTO_CHECK_GT
#define CHECK_GE TS_AUTO_CHECK_GE
#define CHECK_LT TS_AUTO_CHECK_LT
#define CHECK_LE TS_AUTO_CHECK_LE
#define CHECK TS_AUTO_CHECK
#define LOG TS_LOG
#define FATAL LOG_FATAL
#define WARNING LOG_ERROR
#define INFO LOG_INFO

        template <typename T>
        void caffe_copy(const int N, const T *X, T *Y) {
            std::memcpy(Y, X, N * sizeof(T));
        }

        template <typename T>
        void caffe_sub(const int n, const T* a, const T* b, T* y) {
            for (int i = 0; i < n; ++i) {
                *y++ = *a++ - *b++;
            }
        }
        ...
        public:
            ...
            counting_iterator(T val) : m_val(val) {}
            ...
            counting_iterator operator+(T shift) const {
                return self(m_val + shift);
            }
            ...
            T operator-(const self &it) const {
                return this->m_val - it.m_val;
            }
...
// Explicit initialization.
        ...
        void OutputBBox(const NormalizedBBox &bbox, const pair<int, int> &img_size,
            ...
            if (has_resize && resize_param.resize_mode()) {
                ...
                switch (resize_param.resize_mode()) {
                    case ResizeParameter::Resize_mode::WARP:
                        ClipBBox(temp_bbox, &temp_bbox);
                        ScaleBBox(temp_bbox, height, width, out_bbox);
                        break;
                    case ResizeParameter::Resize_mode::FIT_LARGE_SIZE_AND_PAD:
                        ...
                        ScaleBBox(temp_bbox, height, width, out_bbox);
                        break;
                    case ResizeParameter::Resize_mode::FIT_SMALL_SIZE:
                        ...
                        break;
                    default:
                        LOG(FATAL) << "Unknown resize mode." << eject;
                }
            } else {
        ...
        void ExtrapolateBBox(const ResizeParameter &param, const int height,
            ...
            if (height_scale > 0 && width_scale > 0 &&
                param.resize_mode() == ResizeParameter::Resize_mode::FIT_SMALL_SIZE) {
        ...
        bool MeetEmitConstraint(const NormalizedBBox &src_bbox,
            ...
            if (emit_type == EmitConstraint::EmitType::CENTER) {
        ...
        void EncodeBBox(
            ...
            if (code_type == PriorBoxParameter::CodeType::CORNER) {
            ...
            } else if (code_type == PriorBoxParameter::CodeType::CENTER_SIZE) {
            ...
            } else if (code_type == PriorBoxParameter::CodeType::CORNER_SIZE) {
                float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
                        CHECK_GT(prior_width, 0);
                float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
                        CHECK_GT(prior_height, 0);
                ...
                LOG(FATAL) << "Unknown LocLossType." << eject;
            }
        }
        ...
        void MatchBBox(const vector<NormalizedBBox> &gt_bboxes,
            ...
            switch (match_type) {
                ...
                case MultiBoxLossParameter::MatchType::PER_PREDICTION:
                    ...
                    break;
                default:
                    LOG(FATAL) << "Unknown matching type." << eject;
                    break;
            }
        ...
        void FindMatches(const vector<LabelBBox> &all_loc_preds,
            ...
            // Get parameters.
            TS_CHECK(multibox_loss_param.has_num_classes()) << "Must provide num_classes." << eject;
            const int num_classes = multibox_loss_param.num_classes();
            TS_CHECK_GE(num_classes, 1) << "num_classes should not be less than 1." << eject;
...
// Ignore background loc predictions.
        ...
        inline bool IsEligibleMining(const MiningType mining_type, const int match_idx,
            ...
            if (mining_type == MultiBoxLossParameter::MiningType::MAX_NEGATIVE) {
                return match_idx == -1 && match_overlap < neg_overlap;
            } else if (mining_type == MultiBoxLossParameter::MiningType::HARD_EXAMPLE) {
                return true;
            } else {
                return false;
            }
...
// Get parameters.
            ...
            if (mining_type == MultiBoxLossParameter::MiningType::NONE) {
                return;
            }
...
// Compute confidence losses based on matching results.
            ...
            if (mining_type == MultiBoxLossParameter::MiningType::HARD_EXAMPLE) {
...
// Explicite initialization.
        ...
        void GetGroundTruth(const Dtype *gt_data, const int num_gt,
            ...
            for (int i = 0; i < num_gt; ++i) {
                ...
                TS_CHECK_NQ(background_label_id, label)
                        << "Found background label in the dataset." << eject;
                ...
                for (map<int, vector<int> >::const_iterator
                    ...
                    for (int j = 0; j < match_index.size(); ++j) {
                        ...
                        for (int k = 0; k < 4; ++k) {
                            ...
                            if (loc_loss_type == MultiBoxLossParameter::LocLossType::SMOOTH_L1) {
                            ...
                            } else if (loc_loss_type == MultiBoxLossParameter::LocLossType::L2) {
                                loss += 0.5 * val * val;
                            } else {
                                LOG(FATAL) << "Unknown loc loss type." << eject;
                            }
                        }
                        loc_loss[j] = loss;
                        ++count;
        ...
        void ComputeConfLoss(const Dtype *conf_data, const int num,
            ...
            for (int i = 0; i < num; ++i) {
                ...
                for (int p = 0; p < num_preds_per_class; ++p) {
                    ...
                    Dtype loss = 0;
                    if (loss_type == MultiBoxLossParameter::ConfLossType::SOFTMAX) {
                                CHECK_GE(label, 0);
                    ...
                    } else if (loss_type == MultiBoxLossParameter::ConfLossType::LOGISTIC) {
                        int target = 0;
                        for (int c = 0; c < num_classes; ++c) {
                            if (c == label) {
                                target = 1;
                        ...
                        LOG(FATAL) << "Unknown conf loss type." << eject;
                    }
...
// Retrieve parameters.
            ...
            if (multibox_loss_param.has_do_neg_mining()) {
                ...
                CHECK_EQ(do_neg_mining, mining_type != MultiBoxLossParameter::MiningType::NONE);
            }
            do_neg_mining = mining_type != MultiBoxLossParameter::MiningType::NONE;
            const ConfLossType conf_loss_type = multibox_loss_param.conf_loss_type();
            int count = 0;
            for (int i = 0; i < num; ++i) {
                if (all_gt_bboxes.find(i) != all_gt_bboxes.end()) {
        ...
        void GetDetectionResults(const Dtype *det_data, const int num_det,
            ...
            for (int i = 0; i < num_det; ++i) {
                ...
                int label = det_data[start_idx + 1];
                        ...
                        << "Found background label in the detection results." << eject;
                NormalizedBBox bbox;
...
// Explicit initialization.
        ...
        void ApplyNMS(const vector<NormalizedBBox> &bboxes, const vector<float> &scores,
            ...
            // Sanity check.
                    ...
                    << "bboxes and scores have different size." << eject;

            // Get top_k scores (with corresponding indices).
            vector<int> idx(counting_iterator<int>(0),
                            counting_iterator<int>(scores.size()));
        ...
        void ApplyNMS(const bool *overlapped, const int num, vector<int> *indices) {
            vector<int> index_vec(counting_iterator<int>(0),
                                  counting_iterator<int>(num));
            // Do nms.
            indices->clear();
        ...
        void ApplyNMSFast(const vector<NormalizedBBox> &bboxes,
                          ...
                          vector<int> *indices) {
            // Sanity check.
            TS_CHECK_EQ(bboxes.size(), scores.size())
                    << "bboxes and scores have different size." << eject;
        ...
        void ComputeAP(const vector<pair<float, int> > &tp, const int num_pos,
            ...
            TS_CHECK_EQ(tp.size(), fp.size()) << "tp must have same size as fp." << eject;
