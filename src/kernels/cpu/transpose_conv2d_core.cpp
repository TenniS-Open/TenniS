...
namespace ts {
    namespace cpu {
template <typename T>
static void cpu_transpose_conv2d_nchw_compute_run(const Tensor &x, const Padding2D &padding, float padding_value,
...
void Conv2DTransposeCore::conv2d_transpose(const Tensor &x, const Padding2D &padding, float padding_value, const Tensor &w,
                            const Stride2D &stride, const Dilation2D &dilation, Conv2DFormat format, Tensor &out, Stack & stack) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE) \
        case DTYPE: { cpu_transpose_conv2d_nchw_compute_run<TYPE>(x, padding, padding_value, w, stride, dilation, out, stack);; break; }
