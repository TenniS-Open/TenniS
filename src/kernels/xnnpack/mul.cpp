...
namespace ts {
    namespace xnn {
        ...
        int Mul::infer(Stack &stack, std::vector<Tensor::Prototype> &output) {
            auto cpu_mul_infer = OperatorCreator::Create(memory_device().type(), name::layer::mul(), true);
            InferOperator(cpu_mul_infer, stack, 2, output);
            return 1;
        }

        int Mul::run(Stack &stack) {
        ...
        void Mul::mul(const Tensor &lhs, const Tensor &rhs, Tensor &out) {
            ...
            m_status = xnn_setup_multiply_nd_f32(m_op, lhs_shape.size(), lhs_shape.data(), rhs_shape.size(),rhs_shape.data(),
...
using namespace ts;
using namespace xnn;
TS_REGISTER_OPERATOR(Mul, ts::XNNPACK, "xnn::" + name::layer::mul())
